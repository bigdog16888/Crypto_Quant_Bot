...
def test_credit_fill_while_gated(temp_db):
    """Test credit_fill behavior when bot is in REQUIRE_MANUAL_PROOF status."""
    ...
    try:
        ...
        cursor.execute("UPDATE bots SET status = 'REQUIRE_MANUAL_PROOF' WHERE id = 99999")
        temp_db.commit()
        ...
        row_order = cursor.execute("SELECT status, filled_amount FROM bot_orders WHERE id = 999991").fetchone()
        assert row_order[0] == 'filled'
        assert float(row_order[1]) == 0.01
        
        # Verify trades.open_qty accumulator updated
        row_trade = cursor.execute("SELECT open_qty, total_invested, avg_entry_price FROM trades WHERE bot_id = 99999").fetchone()
        ...
        row_bot = cursor.execute("SELECT status FROM bots WHERE id = 99999").fetchone()
        ...
        row_tp = cursor.execute("SELECT status, filled_amount FROM bot_orders WHERE id = 999992").fetchone()
        ...
        row_bot_final = cursor.execute("SELECT status FROM bots WHERE id = 99999").fetchone()
        assert row_bot_final[0] == 'Scanning'
        
    finally:
        # Cleanup handled by temp_db fixture teardown
        pass