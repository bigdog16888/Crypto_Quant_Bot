...
2. Run seal_trade_state() — should sync trades.open_qty from active_positions
3. Verify trades.open_qty matches active_positions
...
def setup_test_db():
    """Create test schema matching production EXACTLY."""
    conn = sqlite3.connect(TEST_DB)
    for table in ['bots', 'trades', 'active_positions', 'bot_orders', 'exchange_fills']:
...
def insert_test_bot(bot_id=10018, name='sui long', pair='SUI/USDC:USDC', direction='LONG'):
    """Insert a test bot matching the SUIUSDC case."""
    conn = sqlite3.connect(TEST_DB)
    conn.execute("""
        INSERT INTO bots (id, name, pair, normalized_pair, direction, is_active, status)
        VALUES (?, ?, ?, ?, ?, 1, 'IN TRADE')
    """, (bot_id, name, pair, normalize_symbol(pair).upper(), direction))
    conn.execute("""
...
def upsert_active_position(bot_id=10018, pair='SUI/USDC:USDC', side='LONG', size=58.6, entry_price=0.706):
    ...
    """, (bot_id, pair, side, size, entry_price))
    conn.commit()
    conn.close()

def test_active_positions_staleness_bug():
    """
    ...
    setup_test_db()
    insert_test_bot()
    upsert_active_position(bot_id=10018, pair='SUI/USDC:USDC', side='LONG', size=58.6, entry_price=0.706)
...
def test_checkpoint_reads_active_positions():
    """
...
if __name__ == '__main__':
    ...
    print()
    print("Test 2: seal_trade_state syncs trades from active_positions (should FAIL before fix)")
    try:
        test_active_positions_staleness_bug()
        print("  PASS ✓ (fix works!)")
    except AssertionError as e:
        print(f"  FAIL ✗ (RED - bug confirmed): {e}")
