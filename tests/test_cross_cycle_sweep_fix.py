...
using REAL bot 10018 data (cycles 6 and 10 are the known unresolved cases).
"""
...
def test_cross_cycle_sweep_fixed():
    ...
    conn = sqlite3.connect('D:/Crypto_Quant_Bot/crypto_bot.db')
    cursor = conn.cursor()
    
    bot_id = 10018
    new_cycle = 26
    now_ts = 1789185774  # wipe_wall_ts for bot 10018
...
def test_classification_with_fixed_logic():
    ...
    for c in classifications:
        if c['cycle'] == 6:
            ...
            assert c['reset_orders'] == 7, f"Cycle 25 should have 7 reset orders, got {c['reset_orders']}"
            assert abs(c['reset_units'] - 543.0) < 0.01, f"Cycle 25 should have 543.0 units, got {c['reset_units']}"
...
if __name__ == '__main__':
    print("=" * 80)
    print("SUI CROSS-CYCLE SWEEP FIX REGRESSION TEST")
    print("=" * 80)
    print()
    ...
    # Test 2: Classification with fixed logic
