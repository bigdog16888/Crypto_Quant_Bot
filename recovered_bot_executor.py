...
def sync_stale_open_orders(bot_id: int, exchange: ExchangeInterface, conn, max_age_seconds: int = 120) -> int:
    ...
    # INV-31: All bot_orders writes route through the WriteQueue singleton.
    # The public function performs reads on the passed connection; each
    # write block (status update, deletion, cancellation cleanup) is
    # encapsulated in an internal function that opens its own connection
    # and is serialized by put_and_wait. Nested put_and_wait calls (e.g.
    # credit_fill -> seal_trade_state) detect the worker thread and run
    # inline — no deadlock. Under pytest the queue bypasses and the
    # internal runs on the calling thread, so test mocks/patches behave
    # exactly as before.
    ...
    for row in rows:
        ...
        try:
                ...
                if status == 'cancelling' and db_status == 'cancelled':
                    ...
                    if db_filled > 0:
                        ...
                        WriteQueue().put_and_wait(
                            _sync_cancel_with_fill_internal,
                            bot_id,
                            row_id,
                            db_filled,
                        )
                        ...
                        WriteQueue().put_and_wait(
                            _sync_cancel_zero_fill_internal,
                            bot_id,
                            row_id,
                        )
                        synced_count += 1
            ...
            if ("notfound" in err_name.lower() or 
                ...
                WriteQueue().put_and_wait(
                    _sync_notfound_cancel_internal,
                    bot_id,
                    row_id,
                )
...
def _sync_cancel_with_fill_internal(bot_id: int, row_id: int, db_filled: float) -> None:
    """
    Internal: mark a cancelling order as filled with the DB fill amount.
    Runs inside WriteQueue serialization — must NOT be called directly;
    use the public sync_stale_open_orders() wrapper.
    """
...
def _sync_cancel_zero_fill_internal(bot_id: int, row_id: int) -> None:
    """
    Internal: delete a cancelling order with zero fill confirmed by exchange.
    Runs inside WriteQueue serialization — must NOT be called directly.
    """
    from engine.database import get_connection
    conn = get_connection()
...
def _sync_status_update_internal(bot_id: int, row_id: int, db_status: str, ex_filled: float, ex_avg_price: float) -> None:
    """
    Internal: generic status/filled/price update for a bot_orders row.
    Runs inside WriteQueue serialization — must NOT be called directly.
    """
    from engine.database import get_connection
    conn = get_connection()
...
def _sync_notfound_cancel_internal(bot_id: int, row_id: int) -> None:
    """
    Internal: mark an order as cancelled when exchange returns NotFound.
    Runs inside WriteQueue serialization — must NOT be called directly.
    """
    from engine.database import get_connection
    conn = get_connection()
...
def enforce_hedge_child_state(child_bot_id: int, conn) -> str:
    ...
    INV-31: All bot_orders/trades writes route through the WriteQueue singleton.
    The public function performs reads on the passed connection; each write
    block is encapsulated in an internal function that opens its own connection
    and is serialized by put_and_wait. Nested put_and_wait calls detect the
    worker thread and run inline — no deadlock. Under pytest the queue bypasses
    and the internal runs on the calling thread, so test mocks/patches behave
    exactly as before.
    """
    from engine.write_queue import WriteQueue
    ...
    if correct_cycle:
        ...
        if cycle_qty > 0.0001:
            if correct_cycle[0] != child_cycle_id:
                ...
                WriteQueue().put_and_wait(
                    _enforce_hedge_cycle_repair_internal,
                    child_bot_id,
                    correct_cycle[0],
                    child_cycle_id,
                )
    ...
    if child_cycle_id != parent_cycle_id and child_qty <= 0.0001:
        WriteQueue().put_and_wait(
            _enforce_hedge_cycle_sync_internal,
            child_bot_id,
            parent_cycle_id,
            child_cycle_id,
        )
...
def _enforce_hedge_cycle_repair_internal(child_bot_id: int, correct_cycle: int, old_cycle: int) -> None:
    """
    Internal: correct child trades cycle_id from bot_orders evidence.
    Runs inside WriteQueue serialization — must NOT be called directly.
    """
    from engine.database import get_connection
    conn = get_connection()
    conn.execute(
        ...
        (correct_cycle, child_bot_id)
    )
    conn.commit()


def _enforce_hedge_cycle_sync_internal(child_bot_id: int, parent_cycle_id: int, child_cycle_id: int) -> None:
    """
    Internal: sync child cycle_id to parent when child is flat/standby.
    Runs inside WriteQueue serialization — must NOT be called directly.
    """
    from engine.database import get_connection
    conn = get_connection()
...
def _cancel_non_tp_orders(bot_id: int, exchange, conn):
    ...
    INV-31: bot_orders writes route through WriteQueue.
    """
    from engine.write_queue import WriteQueue
    try:
        ...
        now_ts = int(time.time())
        WriteQueue().put_and_wait(
            _cancel_non_tp_orders_db_sync_internal,
            bot_id,
            now_ts,
        )
    except Exception as e:
...
def _cancel_non_tp_orders_db_sync_internal(bot_id: int, now_ts: int) -> None:
    """
    Internal: bulk cancel non-TP bot_orders rows.
    Runs inside WriteQueue serialization — must NOT be called directly.
    """
    from engine.database import get_connection
    conn = get_connection()
...
def _reset_to_hedge_standby(child_bot_id: int, conn, parent_cycle_id: int, exchange=None):
    ...
    Phase 1 receipt writes (INSERT + UPDATE) and Phase 2 updates are each
    encapsulated in internal functions serialized by put_and_wait. The
    Phase 1→Phase 2 ordering is preserved by the caller awaiting each
    put_and_wait sequentially.
    """
    from engine.write_queue import WriteQueue
        ...
        if attributed_qty > 0.0001 and pair:
            ...
            if not skip_phase1:
                ...
                if close_qty <= 0.0001:
                    ...
                    # INV-31: receipt write goes through WriteQueue
                    _pending_row_id = WriteQueue().put_and_wait(
                        _reset_phase1_receipt_insert_internal,
                        child_bot_id,
                        close_cid,
                        close_qty,
                        parent_cycle_id,
                    ...
                    try:
                        ...
                        # INV-31: receipt update goes through WriteQueue
                        WriteQueue().put_and_wait(
                            _reset_phase1_receipt_update_internal,
                            _pending_row_id,
                            real_order_id,
                        )
                        logger.warning(
                        ...
                        # INV-31: receipt failure update goes through WriteQueue
                        WriteQueue().put_and_wait(
                            _reset_phase1_receipt_fail_internal,
                            _pending_row_id,
                            str(_close_err),
                        )
    ...
    # ═════════════════════════════════════════════════════════════════════════
    WriteQueue().put_and_wait(
        _reset_phase2_db_update_internal,
        child_bot_id,
        parent_cycle_id,
    )
    ...
    WriteQueue().put_and_wait(
        _reset_phase2_status_force_internal,
        child_bot_id,
    )

    # Audit row (always written, Phase 1 or not)
    # INV-31: save_bot_order already routes through WriteQueue internally
...
def _reset_phase1_receipt_insert_internal(
    child_bot_id: int,
    close_cid: str,
    close_qty: float,
    parent_cycle_id: int,
    ...
    """
    Internal: insert pending reset_close receipt row, return lastrowid.
    Runs inside WriteQueue serialization — must NOT be called directly.
    """
    from engine.database import get_connection as _db_conn
    ...
    return _receipt_cursor.lastrowid


def _reset_phase1_receipt_update_internal(
    row_id: int,
    real_order_id: str,
) -> None:
    """
    Internal: update pending receipt with real exchange order_id → open.
    Runs inside WriteQueue serialization — must NOT be called directly.
    """
    from engine.database import get_connection as _db_conn
    _receipt_conn = _db_conn()
    _receipt_conn.execute(
        ...
        (real_order_id, int(time.time()), row_id)
    )
    _receipt_conn.commit()


def _reset_phase1_receipt_fail_internal(
    row_id: int,
    error_msg: str,
) -> None:
    """
    Internal: mark pending receipt as failed.
    Runs inside WriteQueue serialization — must NOT be called directly.
    """
    from engine.database import get_connection as _db_conn
    _receipt_conn = _db_conn()
    _receipt_conn.execute(
        ...
        (int(time.time()), f"[RESET-P1] Exchange close failed: {error_msg}", row_id)
    )
    _receipt_conn.commit()


def _reset_phase2_db_update_internal(
    child_bot_id: int,
    parent_cycle_id: int,
) -> None:
    """
    Internal: Phase 2 — zero trades, set cycle_id.
    Runs inside WriteQueue serialization — must NOT be called directly.
    """
    from engine.database import get_connection
    conn = get_connection()
...
def _reset_phase2_status_force_internal(
    child_bot_id: int,
) -> None:
    """
    Internal: force status to hedge_standby if seal overwrote it.
    Runs inside WriteQueue serialization — must NOT be called directly.
    """
    from engine.database import get_connection
    conn = get_connection()
    ...
    def _reset_to_hedge_standby_internal(child_bot_id: int):
        """
        Internal: Reset child bot to hedge_standby status.
        Called via WriteQueue.
        """
        from engine.database import get_connection
        conn = get_connection()
        WriteQueue().put_and_wait(
    ...
    def _compute_effective_tp_internal(bot_id: int, tp_price: float):
        """
        Internal: Update trades table with computed effective TP price.
        Called via WriteQueue.
        """
        from engine.database import get_connection
        conn = get_connection()
        conn.execute(
            "UPDATE trades SET target_tp_price = ? WHERE bot_id = ?",
            (tp_price, bot_id)
        )
        conn.commit()

    def _execute_entry_basket_start_internal(bot_id: int, timestamp: int):
        """
        Internal: Record basket start time for entry tracking.
        Called via WriteQueue.
        """
        from engine.database import get_connection
        conn = get_connection()
        conn.execute(
            "UPDATE trades SET basket_start_time = ? WHERE bot_id = ?",
            (timestamp, bot_id)
        )
        conn.commit()

    def _execute_entry_order_id_internal(bot_id: int, order_id: str):
        """
        Internal: Record entry order ID after placement.
        Called via WriteQueue.
        """
        from engine.database import get_connection
        conn = get_connection()
    ...
    def _execute_exit_tp_cancel_tp_internal(bot_id: int, filled_amount: float, order_id: str):
        """
        Internal: Cancel TP order on successful fill.
        Called via WriteQueue.
        """
        from engine.database import get_connection
        conn = get_connection()
        ...
        conn.execute(
            "UPDATE bot_orders SET status = 'cancelled', filled_amount = ? WHERE order_id = ?",
            (filled_amount, order_id)
        )
        conn.commit()

    def _execute_exit_tp_missing_tp_internal(bot_id: int, order_id: str):
        """
        Internal: Mark TP order as missing when not found on exchange.
        Called via WriteQueue.
        """
        from engine.database import get_connection
        conn = get_connection()
        ...
        conn.execute(
            "UPDATE bot_orders SET status = 'missing' WHERE order_id = ?",
            (order_id,)
        )
        conn.commit()

    def _maintain_hedge_cycle_enforce_open_qty_internal(bot_id: int, open_qty: float):
        """
        Internal: Update open_qty during hedge cycle enforcement.
        Called via WriteQueue.
        """
        from engine.database import get_connection
        conn = get_connection()
    ...
    def _maintain_hedge_cycle_delete_step_internal(bot_id: int, step: int, cycle_id: int, client_order_id: str):
        """
        Internal: Delete bot_orders for a specific step during hedge cycle enforcement.
        Called via WriteQueue.
        """
        from engine.database import get_connection
        conn = get_connection()
        conn.execute(
            "DELETE FROM bot_orders WHERE bot_id = ? AND step = ? AND cycle_id = ? AND client_order_id = ?",
            (bot_id, step, cycle_id, client_order_id)
        )
        conn.commit()

    def _maintain_cancel_non_tp_db_sync_internal(bot_id: int):
        """
        Internal: Sync DB after cancelling non-TP orders - clear tp_order_id.
        Called via WriteQueue.
        """
        from engine.database import get_connection
        conn = get_connection()
    ...
    def _maintain_partial_fill_update_qty_internal(bot_id: int, filled_qty: float):
        """
        Internal: Update trades with partial fill quantity.
        Called via WriteQueue.
        """
        from engine.database import get_connection
        conn = get_connection()
    ...
    def _maintain_stuck_dust_phase_internal(bot_id: int):
        """
        Internal: Mark bot cycle_phase as STUCK_DUST_NO_EXIT.
        Called via WriteQueue.
        """
        from engine.database import get_connection
        conn = get_connection()
    ...
    def _maintain_margin_held_clear_internal(bot_id: int):
        """
        Internal: Clear MARGIN_HELD cycle_phase back to ACTIVE.
        Called via WriteQueue.
        """
        from engine.database import get_connection
        conn = get_connection()
        conn.execute(
            "UPDATE trades SET cycle_phase = 'ACTIVE' WHERE bot_id = ? AND cycle_phase IN ('MARGIN_HELD', 'MARGIN_HELD_RECOVERY')",
            (bot_id,)
        )
        conn.commit()
    ...
    def _maintain_margin_held_set_internal(bot_id: int):
        """
        Internal: Set cycle_phase to MARGIN_HELD.
        Called via WriteQueue.
        """
        from engine.database import get_connection
        conn = get_connection()
        conn.execute(
            "UPDATE trades SET cycle_phase = 'MARGIN_HELD' WHERE bot_id = ? AND cycle_phase NOT IN ('CARRY_LONG', 'CARRY_SHORT')",
            (bot_id,)
        )
        conn.commit()
    ...
    def _maintain_entry_confirmed_internal(bot_id: int):
        """
        Internal: Mark entry as confirmed in trades table.
        Called via WriteQueue.
        """
        from engine.database import get_connection
        conn = get_connection()
    ...
    def _maintain_grid_order_clear_internal(bot_id: int):
        """
        Internal: Clear grid_order_id in trades table.
        Called via WriteQueue.
        """
        from engine.database import get_connection
        conn = get_connection()
    ...
    def _signal_hedge_cycle_carry_orders_internal(child_bot_id: int, new_cycle_id: int, old_cycle_id: int):
        """
        Internal: Carry forward bot_orders to new cycle during hedge signal.
        Called via WriteQueue.
        """
        from engine.database import get_connection
        conn = get_connection()
        conn.execute(
            ...
            (new_cycle_id, child_bot_id, old_cycle_id)
        )
        conn.commit()

    def _signal_hedge_cycle_sync_trades_internal(child_bot_id: int, new_cycle_id: int):
        """
        Internal: Sync trades.cycle_id to parent cycle during hedge signal.
        Called via WriteQueue.
        """
        from engine.database import get_connection
        conn = get_connection()
        conn.execute(
            "UPDATE trades SET cycle_id = ? WHERE bot_id = ?",
            (new_cycle_id, child_bot_id)
        )
        conn.commit()

    def _signal_hedge_recon_open_qty_internal(child_bot_id: int, corrected_qty: float):
        """
        Internal: Update open_qty during hedge reconciliation.
        Called via WriteQueue.
        """
        from engine.database import get_connection
        conn = get_connection()
    ...
    def _signal_hedge_recon_delete_order_internal(child_bot_id: int, step: int, cycle_id: int, client_order_id: str):
        """
        Internal: Delete reconciled bot_order during hedge signal.
        Called via WriteQueue.
        """
        from engine.database import get_connection
        conn = get_connection()
        conn.execute(
            "DELETE FROM bot_orders WHERE bot_id = ? AND step = ? AND cycle_id = ? AND client_order_id = ?",
            (child_bot_id, step, cycle_id, client_order_id)
        )
        conn.commit()
    ...
    def _compute_effective_tp(self, bot_id: int, name: str, bot_status: dict,
        ...
        try:
            ...
            if decayed_tp != stored_rounded:
                ...
                try:
                    WriteQueue().put_and_wait(
                            _compute_effective_tp_internal,
                            bot_id,
                            decayed_tp,
                        )
            ...
            if config.DRY_RUN:
                ...
                try:
                    ...
                    try:
                        ...
                        cursor = conn.cursor()
                        WriteQueue().put_and_wait(
                            _execute_entry_basket_start_internal,
                            bot_id,
                            int(time.time()),
                        )
                        # Note: bots status update still direct (not trades/bot_orders)
                    ...
                    if order:
                        ...
                        try:
                            WriteQueue().put_and_wait(
                            _execute_entry_order_id_internal,
                            bot_id,
                            order['id'],
                        )
            ...
            try:
                ...
                if order_status:
                    ...
                    if status == 'filled' or (status == 'closed' and filled > 0 and filled >= amount * 0.99):
                        ...
                        WriteQueue().put_and_wait(
                            _execute_exit_tp_cancel_tp_internal,
                            bot_id,
                            filled,
                            tp_order_id,
                        )
                ...
                if "not found" in err_msg or "-2013" in err_msg or "invalidorder" in err_msg:
                    ...
                    WriteQueue().put_and_wait(
                        _execute_exit_tp_missing_tp_internal,
                        bot_id,
                        tp_order_id,
                        )
                else:
                    ...
                    if parent_row:
                        ...
                        if aggregate_drift < -tolerance:
                            if child_status == 'REQUIRE_MANUAL_PROOF':
                                ...
                                # Handled by individual WriteQueue calls below
