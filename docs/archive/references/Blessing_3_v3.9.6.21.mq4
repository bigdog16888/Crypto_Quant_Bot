...
input string             Version_3_9_6_20      = "EA Settings:";
input string             TradeComment          = "b3_v396.21";
input string             Notes                 = "";
...
double OnTester()
{
   ...
   return (Balance * Balance * Trades * MinDD);
}
...
double portion_maxlots()
{
   ...
   // Check if the margin required is zero
   if (marginRequired == 0)
   {
       Print("Error: Margin required is zero for symbol ", Symbol());
       return(1);  // Returning zero or handle as per your requirement
   }

   // Perform the division safely
   return(PortionBalance / marginRequired);
}
