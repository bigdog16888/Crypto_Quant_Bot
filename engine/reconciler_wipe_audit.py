...
def _check_recompute_for_suspects(cursor, bot_id: int, symbol: str) -> List[Dict[str, Any]]:
    """
...
def check_recompute_for_suspects(bot_id: int, symbol: str) -> List[Dict[str, Any]]:
    """
    Public API: checks for suspect reset_cleared rows without proof snapshots.
    Uses WriteQueue to execute read query on the write queue worker thread.
    """
    from engine.write_queue import WriteQueue
    return WriteQueue().put_and_wait(_check_recompute_for_suspects, bot_id, symbol)


def _audit_bot_wipes(cursor, bot_id: int, symbol: str, exchange_gap: float) -> WipeAuditResult:
    """
    Internal: audits a specific bot for unproved wipes that might explain a reconciliation gap.
    """
...
def _system_wipe_health_check(cursor, active_bots: List[Any]) -> List[WipeAuditResult]:
    """
    Internal: system-wide audit of all unproved reset_cleared rows.
    """
    results = []
    for bot in active_bots:
        ...
        audit = _audit_bot_wipes(cursor, b_id, pair, exchange_gap=0.0)
...
def system_wipe_health_check(active_bots: List[Any]) -> List[WipeAuditResult]:
    """
    Public API: system-wide wipe health check. Uses WriteQueue.
    """
    from engine.write_queue import WriteQueue
    return WriteQueue().put_and_wait(_system_wipe_health_check, active_bots)


def _resolve_suspect_row(cursor, order_id: str, resolution_type: str):
    """
    Internal: updates a suspect row with a resolution note to clear it from the audit.
...
def resolve_suspect_row(order_id: str, resolution_type: str):
    """
    Public API: resolves a suspect row. Uses WriteQueue.
    resolution_type: 'forensic_adopt', 'force_sl', 'manual_reset'
    """
    from engine.write_queue import WriteQueue
    WriteQueue().put_and_wait(_resolve_suspect_row, order_id, resolution_type)