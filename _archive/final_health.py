...
def final_health_check():
    ...
    # 2. Fetch DB Trades
    trades = pd.read_sql_query("SELECT bot_id, symbol, current_step, total_invested FROM trades", conn)
    ...
    for _, row in trades.iterrows():
        ...
        if sym not in active_pos:
            ...
            if abs(e_size * float(active_pos[sym]['entryPrice']) - invested) > 5.0:
    ...
    for sym, p in active_pos.items():
        if sym not in trades['symbol'].values:
