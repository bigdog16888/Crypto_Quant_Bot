...
- Test gate: 2026-09-02 (t_d0a8526f, at tip f79ea2a after merge 7766bf3 of d53937b): bare pytest Py310 in clean worktree = 582 collected, 0 collection errors; full suite 559 passed / 23 failed / 4 subtests passed (178.88s). HONESTY NOTE: the earlier claim here (516/19, "collection errors FIXED") was PREMATURE — a63ceac contained NO deletion hunk; the test_verify_fill_on_exchange.py deletion existed only as an unstaged working-tree change in the mid-merge checkout, and a SECOND uncommitted deletion (tests/test_hedge_watchdog_netting.py, auto-staged by the merge from d53937b's rejected netting API) was also a live collection error. Both deletions actually committed: b2af64f (verify_fill) + f79ea2a (hedge_watchdog_netting). Failure decomposition (verified by targeted runs both directions): 23 = 19 pre-existing at 4d3ba20 (inv31×4 closure-signature class, inv38, parity_gates_retry, require_proof_writers×2, seal_short_phantom, snap_allocate_gate, ghost_clearing×2, hedge_lifecycle, auto_repair_guards, streamlit, adopt_fill_guard×4) + 4 NEW introduced by merge commit 7766bf3 itself (they PASS at 2870929, FAIL at 7766bf3, unchanged by the deletion commits): test_inv31_ground_truth_reconciler ghost_not_healed_when_orders_exist / in_sync_no_action / orphan_physical_logged_not_healed + test_inv17 gtr_clears_stuck_pending_placement. All 4 new are GTR/WriteQueue-signature class ("_reset_hedge_close() missing 1 required positional argument: 'bot_id'") — same class the inv31-writequeue branch (ed1d8cd, separate worktree) is fixing.
...
- [TEST DEBT] RESOLVED 2026-09-02: collection errors fixed for real via t_d0a8526f commits b2af64f + f79ea2a (the earlier t_33caaa9d "RESOLVED" claim was premature — deletions were uncommitted). Bare pytest at tip f79ea2a collects 582 / 0 errors. REMAINING (open, NOT collection): 19 pre-existing failures at 4d3ba20 + 4 NEW merge-introduced (7766bf3) GTR/WriteQueue closure-signature failures (inv31 ×3, inv17 ×1) — pass at 2870929, fail from 7766bf3 onward; same class inv31-writequeue branch (ed1d8cd) is fixing.
