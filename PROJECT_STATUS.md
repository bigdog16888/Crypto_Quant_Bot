...
**Last updated: 2026-09-10 14:50** | **Engine: RUNNING** from `2e4ca7e` (= `e68e9e1` silent-cancel-swallow P1 fix + this doc), up since 2026-09-10 14:06:29 TRADING (PID 22444, `py scripts/restart_engine.py`). Merge: ff-merge `e68e9e1` into live tree 14:00 (direct descendant of c3acc98, `git diff e68e9e1 HEAD` empty at merge time). Graceful stop of prior instance 13:58 (seal 20/20, last_shutdown.ts 1789019914, SocketLock released). **30-min STABIL-WATCH 14:06→14:36 PASS** (evidence: 114 cycles @ ~12s cadence, zero O-3/breaker/watchdog/RMP events, escalation limited to the known corrupted order 583851054 only, DB row left open correctly — no new cancel marked cancelled, normal TP re-placements flowing [SYNC] sui/sol/btc/gold 14:18–14:32, no ghost stacking on 10019, 232 verify-GETs + 4 confirmed cancels on the healthy path, 7 transient demo-FAPI OHLCV/Price GET errors 14:27–14:29 [USDC-pair market-data flake, self-recovered, none before restart — new observation class to watch, not engine defect]). Backlog confirmed SEPARATE: XAU ORDER-SYNC credit loop logged as its own open P2 (line 7 of this doc).

## Live State 2026-09-10 14:00 (all verified)
- **P1 silent-cancel-swallow FIXED + MERGED (e68e9e1, this session)**: `_raw_request` no longer collapses -2011/-2013 already-gone 400s to None (typed `OrderAlreadyGoneError`, raw body always logged at WARNING); `cancel_order` is a trichotomy (dict=confirmed / None=verified-gone / `CancelFailedError`=neither — verify-by-GET before declaring gone, because XAU 583851054 proved the exchange can return -2011 for hours on a live NEW order); consecutive-failure streak per order id → CRITICAL `[CANCEL-ESCALATION]` at 3 (config `CANCEL_STREAK_ESCALATION`); stale-sweep/TP-SYNC/ghost-evict sites mark DB only on confirmed outcome; REL-1 emergency sweep per-order failure-tolerant. Tests: `tests/test_silent_cancel_swallow.py` RED 7f/3p on c3acc98 → GREEN 10/10 on fix (exact SUI + XAU incident shapes replayed); siblings 68/68; full suite 624p/13f/6e — failure set byte-identical to pre-existing baseline (adopt_fill_guard x4, auto_repair_guards, ghost_clearing x2, parity_gates_retry, require_proof_writers x2, seal_short_phantom, snap_allocate_gate, streamlit test_database_views + freeze_guard x6 teardown). Engine gracefully stopped 13:58:34 (seal 20/20, 0 corrected), merged ff, restart imminent.
- Backlog 2026-09-10 **[OPEN — P2, XAU ORDER-SYNC credit loop — SEPARATE item, NOT fixed by the cancel fix]**: `[ORDER-SYNC] ... new partial fill of 0.014 (cumulative 0.022)` logged ~300× in the 13:39–14:29 Sep-9 window while DB `filled_amount` stayed 0.008 — the partial-fill CREDIT never landed, a different subsystem (ledger credit path, not the cancel path). Same API-corruption window as the SUI cancel loop, but the credit-loop mechanism is distinct and unfixed: the ORDER-SYNC path detected the fill, repeatedly, and the ledger write kept failing without escalation. Root cause not yet investigated. This remains open — the cancel-fix commit (e68e9e1) explicitly does NOT cover it. Needs its own root-cause session: replay the [ORDER-SYNC] credit path against a failing exchange, find where the credit write is swallowed, add escalation identical in spirit to the cancel fix, scenario test.
- Backlog 2026-09-09 (carry-over, status unchanged): `_signal_hedge_child_entry` places child hedge entries without checking child `is_active` (see 09-09 entry below for full incident).
- Backlog: LIVE_GUARD_INV30 marker-row double-count class (100317 09-08; bot_executor.py:691 area).
- **Open class item (queued for branch): stale-cycle_id on downtime-credit path** — PRE-COMMIT-RESOLVE credits TP fills without advancing cycle_id → DEDUP wedge (hit 09-07 four bots, 09-09 10016; both healed manually). Root fix: downtime-credit routes through reset_bot_after_cycle (same as in-session TP hits), + DEDUP-GUARD could self-advance on unambiguous own-terminal-cycle collision. Tests on branch, not live tree.
- REL-1 emergency-path exposure to silent cancel failures: **CLOSED by e68e9e1** — cancel_orders_by_bot_id now per-order outcome-aware (failed cancel not counted, sweep continues, no abort); covered by TestREL1EmergencySweep in the same test file. The 09-09 check-request is answered.
- **[2026-09-10] DOCUMENTED-ACCEPTED test-infra failures (7 of the 13f+6e baseline, accepted per operator 2026-09-10 — engine code, not test code, is unaffected)**: (a) `test_freeze_guard_scenario.py` ×6 errors — pure teardown `PermissionError WinError 32` (Windows temp-dir file lock); all 6 guard assertions themselves PASS (rerun 2026-09-10 in e68e9e1-identical worktree: **9 passed, 6 errors** — errors are tmpdir cleanup only); guard logic verified live in engine.log (FREEZE-GUARD blocks firing correctly on frozen ETH/LINK). (b) `test_streamlit_smoke.py::test_database_views` — passes in isolation (**3 passed, 3 warnings**, 2026-09-10 rerun) but fails only in full-suite ordering = test-isolation artifact, UI-dashboard harness, zero engine involvement; the 3 warnings are `return`-instead-of-`assert` test hygiene, trivial. Remaining unaccepted (money-path, own sessions queued): adopt_fill_guard ×4, snap_allocate_gate, seal_short_phantom, require_proof_writers ×2, ghost_clearing ×2, parity_gates_retry, auto_repair_guards = 12 failures.
- **[2026-09-10] Rate-limit weight map** — `docs/RATE_LIMIT_WEIGHT_MAP.md` (live-measured: real FAPI 2400 weight/min vs demo 6000 = 2.5× gap; normal ops ≤~20% of real cap; N-order cancel-storm = the real rate hazard → back-off work assigned to operability branch, see doc §6).
- ETH/LINK resumption: operator's call. ETH orphan saga closed +$87.56.
- Full suite at e68e9e1: 624p/13f/6e (13f+6e = identical pre-existing set; +27 passes = 10 new cancel tests + 17 from suites added since d5b64ea).
...
- Backlog 2026-09-09 URGENT-CLASS **[P1 — FIXED 2026-09-10 at e68e9e1, see Live State above for full evidence; this entry is the original 09-09 write-up kept for history]**: **silent-cancel-never-lands defect** — `🔥 Cancelled stale CQB_10018_TP_20_3_R1788921114 (No fill, set to cancelling for 1-cycle buffer)` logged 300× over 50 min (13:39–14:29) while the order NEVER actually cancelled on the exchange; no error surfaced, no escalation, the loop just re-flagged "stale" and retried every ~10s. Direct cause of tonight's SUI over-sell: the oversized TP (54.8 vs 21.1 position) filled during downtime because its cancel never landed. A cancel that doesn't succeed must never log as if it did, and must escalate after N failures — not loop silently. CORRELATED SAME-WINDOW SIGNATURE: XAU GRID_14_7 partial-fill credit loop also ran ~300× in the same 13:39–14:29 window (`[ORDER-SYNC] ... new partial fill of 0.014 (cumulative 0.022)` every ~10s, DB stayed at 0.008 — the credit never landed either) while the ghost-sweep NameError was aborting maintain for 10016/10019 (300× each). **[XAU credit-loop portion: STILL OPEN — see the separate P2 entry at the top of Live State. NOT fixed by e68e9e1.]** Two different subsystems silently looping + one crash loop, all in one window — the cancel half is now fixed and tested; the credit half is queued as its own root-cause item. The emergency-liquidation path (REL-1) exposure is CLOSED by the same commit (per-order outcome-aware sweep, TestREL1EmergencySweep).
- Backlog check-request 2026-09-09: confirm whether REL-1 emergency-liquidation path can hit the same silent-cancel-failure shape — **ANSWERED 2026-09-10: YES it could, and e68e9e1 closes it** (same primitive, now outcome-aware; see TestREL1EmergencySweep in tests/test_silent_cancel_swallow.py).
- ETH/LINK resumption: operator's call. ETH orphan saga closed +$87.56.
