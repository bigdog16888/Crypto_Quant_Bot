...
- Last verified: 2026-09-03 15:10 (pre-restart, O-3 sensor fix applied): config effective values = 8 frozen bots (6 ETH + 2 LINK), frozen-bot DB states REQUIRE_MANUAL_PROOF intact, ETH 0.904 orphan untouched on exchange (live pull: 0.901 @ 2405.95, uPnL +86.83), LINK flat. O-3 drawdown sensor FIXED (restart-gap isolation + median-of-top-3 peak): both 2026-09-03 restart deaths traced to a pre-restart 33,931.47 single-tick equity spike + 21h engine-off gap poisoning the 24h window — false 42.4% fire. Post-fix replay on the same live data: drawdown 0.00%, no trip. True exchange equity 17,112.21 (cash 8,692.77 + positions 7,973.83 + uPnL 445.61) vs DB-reported 14,845.53 — the ~2,267 gap is the unowned 0.904 ETH orphan, NOT a loss; cash rose +19.60 across the whole window. DRAWDOWN_PCT stays 20 (operator decision 2026-09-03): the fixed sensor only trips on genuine within-run moves.
...
- Ongoing job (Track A, Hermes): RESTART IN PROGRESS — engine start from 37058b7 with ETH/LINK excluded, watching for FREEZE-GUARD live engagement, SOL/XAU reconcile.
